...
| 属性 | 值 | 不设的后果 |
|---|---|---|
| `DWMWA_CAPTION_COLOR` (35) | 取到的主色（可往中性灰拉一点） | 标题栏还是系统色 |
| `DWMWA_TEXT_COLOR` (36) | 按底色亮度二选一 | **浅底上白字，读不了** |
| `DWMWA_USE_IMMERSIVE_DARK_MODE` (20) | 按底色亮度 | 系统绘制的关闭按钮/边框高光配不上色 |
| `DWMWA_BORDER_COLOR` (34) | 同底色 | 边框出现一条异色线 |

**文字色判据用 BT.601 加权亮度** `0.299R + 0.587G + 0.114B`，阈值 128。
不能用简单的 RGB 均值——人眼对绿色最敏感、蓝色最不敏感，
均值会把深蓝判成浅色，结果就是白底白字。

**主色往中性灰拉 35%** 再上色（`config: titlebar_mute`）。
直接用界面原色往往太饱和，标题栏会抢视线；混一点深灰让它"退后"一格。

#### 时序：为什么延迟 4 秒

`loadFinished` 只代表 DOM 就绪，**样式和首次绘制可能还没完成**——
这时候截到的是白色空白页，取出来的主色是白的。
所以用 `QTimer.singleShot(4000, ...)` 等界面真的画出来。

**只取一次，不做持续监听。** `grab()` 会触发完整的渲染回读，是重操作；
而 DSH 的主题在实际使用中基本不变。
...
所以分两轮：**只要存在有彩像素，就完全忽略灰像素**；纯灰界面才退回灰色方案。

两个被实测推翻的中间方案，留作前车之鉴：

| 方案 | 为什么不行 |
|---|---|
| 灰像素降权 0.5 | 2:1 的像素数优势仍让灰胜出（实测取到 `(24,24,24)`） |
| 要求"有彩像素 ≥ 24 个" | 统计在降采样图上做，真实界面主题色往往只占几十个采样点 → 误判「无彩」退回灰色，正好丢掉要保的信号 |
