...
官方同时维护**三个** dist-tag。它们是三条并行推进的线，不是"稳定／预览"两级：

| tag | 语义 | 2026-09-24 实测值 | 可见条件 |
|---|---|---|---|
| `latest` | 稳定通道 | 0.1.5-rc.3 | 默认跟随 |
| `next` | 候选通道（下一正式版的备选，RC 线） | 0.1.7-rc.2 | 勾选「加入预览计划」后 |
| `alpha` | 尝鲜通道（更早期的测试线） | 0.1.7-alpha.2 | 同上 |

「加入预览计划」是显式 opt-in（对应托盘的可勾选菜单项）。开启后在
`latest / next / alpha` 三者中取**版本最高**的一个；关闭后只跟 `latest`。

**取"最高"而不是写死优先某个 tag，是有代价换来的结论：**

- 2026-09 之前这里只读 `latest` + `alpha`，把 `next` 标成"暂不暴露"。
  上游随后把 RC 线放在 `next` 上，于是 `0.1.7-rc.1` / `0.1.7-rc.2`
  **整条 RC 线在界面上完全不可见**——已装 `0.1.7-alpha.2` 的用户
  被告知"已是最新"，而官方其实已经连发两个更晚的候选版。
- 通道之间的先后顺序会随上游发布流程变化，写死顺序迟早再错一次。
  所以代码里 `PRERELEASE_TAGS` 只声明"哪些 tag 算预览"，高低交给版本号比较。
- 特殊情形：若预览线反而**落后**于稳定线（提版顺序变过），仍然只给稳定版，
  免得"开启预览"变成"被降级"。

关闭预览计划时，托盘若发现当前版本高于稳定版，出现「回归稳定版 \<版本\>」，
一键切回——**没有方便的回滚，预览计划就不成立**。

另外，凡是"跟稳定版"的路径（含「已是最新」提示和「回归稳定版」对话框），
都必须顺带报出预览通道上是否有更高版本。**只说"已是最新"是不诚实的**，
这正是 RC 线被漏掉时用户的体感。
...
| 脚本 | 覆盖范围 | 用哪个解释器 |
|---|---|---|
| `tools/contract_selftest.py` | 安装定位、prefix 层级、通道查询、启动/就绪/URL/停止全链路，以及环境自检与探测回归（node 与 bin.js 分离、显式覆盖绕过缓存、私有 prefix 优先级）。**通道判定离线回归**（`channel_logic_checks()`，用固定 tag 组合，**精简模式也跑**——它是唯一进 CI 的通道守卫） | 任意 Python 3.10+ |
...
| 证据 | 内容 |
|---|---|
| 仓库目录 | `apps/desktop`（Electron 壳）+ `apps/desktop-host`，均在本体 monorepo 内 |
| 包名／版本 | `@deepseek-ai/dsh-desktop`，`"private": true`，版本号与 `@deepseek-ai/dsh` **同步**（当前 0.1.7-rc.2） |
| 官方 release notes | 0.1.7-rc.1/rc.2 里大量桌面端条目：首次使用引导、关窗后台运行、托盘、快捷键、安装包启动修复、macOS 拖窗…… |
| 打包 | electron-builder，Windows NSIS + macOS ICNS/DMG，硬签名（硬件令牌 + 时间戳），appId `com.deepseek.harness` |
| 内置运行时 | 自带独立分发的 Python、Node.js、pnpm；Office 技能默认注册 |
| 进程边界 | 独占 `$DSH_HOME/profiles/desktop`，单实例锁；与 CLI 共享产品数据但不共享可执行包 |

但它**目前没有公开分发入口**：

- `private: true` → 不发 npm；
- GitHub Release **零附件资产**（实测 0 个）；
- 官网 https://deepseek.com/harness 只给 `npx` 与源码两条路；
- `electron-builder` 的 publish 用 `{provider:'generic', url: update.publicUrl}`，
  地址由**构建期环境变量**注入（`.env.windows.example` 里的
  `DOWNLOAD_PROD_COS_BUCKET` 等），即走自有 COS 通道。

> ⚠️ 需要注意区分：公网上已出现 `deepseekdsh.com`、`deepseekagent.io`、
> `github.com/emomg/deepseek-harness-desktop` 等"DSH Desktop"站点，
> 它们**自称社区项目、明确声明非官方**（有的还捆着 0.1.5-rc.1 这种旧内核）。
> 不要把这类站点当成官方发行渠道，更不要照它改我们的设计。

### 11.2 对本项目的判断：不寿终正寝，但要重新定位

官方桌面端确实覆盖了本项目最核心的那件事——**把 Web UI 装进原生窗口、
不用让用户自己装 Node**。但它没覆盖本项目真正花力气的地方：

| 本项目的着力点 | 官方桌面端 | 结论 |
|---|---|---|
| 复用**用户已装**的 DSH（含其 DSH_HOME、已登录态、已有会话） | 独占 `profiles/desktop`，自带全套运行时，与 CLI 的包不共享 | 定位不同，不是替代 |
| 更新只走官方 npm 通道 + 失败自动回滚 + 数据指纹前后比对 | 整体作为签名单元更新，dsh 版本不可单独选 | 本项目能"锁版本/回滚到任意版本" |
| 通道可见性（latest/next/alpha 三条线都摊开给用户看） | 由服务端更新策略决定，用户不选版本 | 本项目更透明 |
| 单 exe、体积小、秒级启动 | 自带 Python+Node+pnpm，安装包体积大 | 本项目轻 |
| 界面层零耦合（对 DSH 的假设全收在 `contract.py` / `compat.py`） | 官方壳与 dsh 同版本绑定，可以随便用内部细节 | 本项目在 DSH 破坏性更新下更抗打 |

**同时必须承认的差距**（官方桌面端已经做到而我们没有）：

- 原生托盘 + 关窗后台运行 + 退出前任务影响确认
- 快捷键系统（可查看/搜索/自定义/恢复）+ 侧栏同步显示键位
- 内置 Python/Node/pnpm 运行时，真正**零依赖**（我们首次运行要装 Node ~13 分钟）
- 硬签名安装包 + 安装器接管退出
- macOS 支持（我们只有 Windows）

**所以真正的风险不是"被替代"，而是两条：**

1. **上游可能停止公开 npm 分发**——若桌面端成为唯一推荐形态，
   `dsh web` 这条路可能降级或被弱化，本项目赖以生存的契约
   （`dsh web --port 0 --no-open` 的 stdout URL）就有消失风险。
2. **重复投入**——若官方桌面端将来公开下载，本项目不该再去追平
   托盘/快捷键/内置运行时这些已被覆盖的能力。

### 11.3 建议的取舍（待老大定）

- **保留**：更新事务（锁版本、回滚、数据指纹）、通道可见性、
  契约层与适配表的解耦设计。这些是官方壳明确不做的。
- **停止投入**：托盘/快捷键/自绘标题栏这类"壳的体验功能"的继续打磨——
  收益会随官方桌面端公开而递减。
- **明确边界**：把自己定位成**"给已有 DSH 环境套一层可控的窗口"**，
  而不是"另一个 DSH 发行版"。前者是差异化，后者是硬碰硬。
- **持续做**：每次 DSH 更新后跑 `tools/contract_selftest.py`。
  上游越强，本项目的价值越集中于"契约探测"这一件事。
